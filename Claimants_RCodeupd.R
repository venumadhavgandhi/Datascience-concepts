...
model <- glm(naffairs~.,data=claimants,family = "binomial")
summary(model)
...
claimants1[,"prob"] <- prob
claimants1[,"pred_values"] <- pred_values
claimants1[,"yes_no"] <- yes_no

View(claimants1[,c(1,7:9)])

table(claimants1$ATTORNEY,claimants1$pred_values)
...
library(ROCR)
rocrpred<-prediction(prob,claimants1$ATTORNEY)
